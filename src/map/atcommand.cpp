...
ACMD_FUNC(mobinfo)
{
	unsigned char msize[SZ_ALL][8] = { "Pequena", "Medio", "Grande" };
	unsigned char mrace[RC_ALL][13] = { "Sem forma", "Morto-vivo", "Fera", "Planta", "Inseto", "Fish", "Demônio", "Demi-Humano", "Anjo", "Dragão", "Jogador" };
	unsigned char melement[ELE_ALL][9] = { "Neutro", "Suiton", "Doton", "Katon", "Fuuton", "Veneno", "Hyouton", "Raiton", "Bakuton", "Jinton", "Shakuton" };
	...
	for (k = 0; k < count; k++) {
		unsigned int j,base_exp,job_exp,hiden_exp;
		mob = mob_db(mob_ids[k]);
		base_exp = mob->base_exp;
		job_exp = mob->job_exp;
		hiden_exp = mob->hiden_exp;
		...
		clif_displaymessage(fd, atcmd_output);
		sprintf(atcmd_output, msg_txt(sd,1242), mob->lv, mob->status.max_hp, base_exp, job_exp, hiden_exp, MOB_HIT(mob), MOB_FLEE(mob)); //  Lv:%d  HP:%d  Base EXP:%u  Job EXP:%u Hiden EXP:%u HIT:%d  FLEE:%d
		clif_displaymessage(fd, atcmd_output);
...
#endif
				if (sd->sc.data[SC_ITEMBOOST])
					dropchance += (dropchance * sd->sc.data[SC_ITEMBOOST]->val1) / 100;
