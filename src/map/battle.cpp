...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	if( sc && sc->count ) { // SC_* that reduce damage to 0.
		...
		if (((sce = sc->data[SC_UTSUSEMI]) || sc->data[SC_BUNSINJYUTSU]) && (flag&BF_WEAPON || flag&BF_MAGIC) && !(skill_get_inf3(skill_id)&INF3_NO_EFF_CICADA)) {
			...
			if (sce) {
				...
				md = mob_once_spawn_sub(bl, bl->m, bl->x, bl->y, status_get_name(bl), MOBID_ZANZOU, "", SZ_SMALL, AI_NONE);
				if (md)
				{
					md->master_id = bl->id;
					...
					mob_spawn(md);
					map_foreachinallrange(unit_changetarget, bl, AREA_SIZE, BL_MOB, bl, &md->bl);
				}
				skill_blown(src,bl,sce->val3,-1,BLOWN_NONE);
				sc_start(src, bl, SC_HIDING, 100, skill_lv, skill_get_time2(KO_ZANZOU, skill_lv));
			}
		...
		if(sc->data[SC_FOGWALL] && skill_id != RK_DRAGONBREATH_WATER) {
		...
		if(sc->data[SC_BITE] || sc->data[SC_ANKLE] || sc->data[SC_ELECTRICSHOCKER]) {
...
static void battle_calc_multi_attack(struct Damage* wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if( sd && !skill_id ) {	// if no skill_id passed, check for double attack [helvetica]
		short i;
		if( ( ( skill_lv = pc_checkskill(sd,TF_DOUBLE) ) > 0 && sd->weapontype1 != W_BOW && sd->weapontype1 != W_REVOLVER && sd->weapontype1 != W_GRENADE)
...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	if(sc) {
		...
		if (sc->data[SC_CONCENTRATION] && skill_id != RK_DRAGONBREATH_WATER)
	...
	// ------------------------------------------------------------------
		case AS_SONICBLOW:
			skillratio += skill_lv * 300;
			break;
		case CH_PALMSTRIKE:
			skillratio += skill_lv * 375;
			break;
		case LG_RAGEBURST:
			skillratio += skill_lv * 375;
			break;
		...
		case TK_JUMPKICK:
			...
			break;
		case RG_RAID:
			skillratio += skill_lv * 250;
			break;
		case KN_BOWLINGBASH:
			skillratio += skill_lv * 175;
			break;
		case PA_PRESSURE:
			skillratio += skill_lv * 375;
			break;
	...
	// Hyuuga
		...
		case MO_CHAINCOMBO:
			...
			break;
		case MO_COMBOFINISH:
			skillratio += 425 * skill_lv;
			break;
		case CH_TIGERFIST:
			skillratio += 675 * skill_lv;
			break;
		case CH_CHAINCRUSH:
	...
	// Kazekage
		...
		case MER_QUICKEN: // Milhares de Mãos
		...
		case MER_TENDER: //Ataque em Area
			skillratio += 1000 * skill_lv;
			break;
	// ------------------------------------------------------------------
	// Portoes
	...
	// Aburame
		case HT_BLITZBEAT:
			skillratio = 625 * skill_lv;
			break;
		case SN_FALCONASSAULT:
			skillratio += 1750 * skill_lv;
			break;
...
static void battle_calc_attack_gvg_bg(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	if( wd->damage + wd->damage2 ) { //There is a total damage value
		if( src != target && //Don't reflect your own damage (Grand Cross)
			(!skill_id || skill_id ||
			(src->type == BL_SKILL && (skill_id == SG_MOON_WARM || skill_id == SG_STAR_WARM))) ) {
...
#endif
		if( skill_id == CR_SHIELDBOOMERANG ) { //Refine bonus applies after cards and elements.
			short index = sd->equip_index[EQI_HAND_L];
...
#define MATK_ADD(a) { ad.damage += a; }
		...
		switch (skill_id) {
			...
			default: {
				...
				switch(skill_id) {
					...
					//---------------------------------------------------------------
					case MG_FIREBOLT:
						(skill_lv < 11 ? mestrar = 250 : mestrar = 300);
						skillratio += skill_lv * mestrar;
						break;
					case MG_FIREBALL:
						(skill_lv < 11 ? mestrar = 200 : mestrar = 267);
						skillratio += skill_lv * mestrar;
						break;
					case NJ_KOUENKA:
						(skill_lv < 11 ? mestrar = 200 : mestrar = 267);
					...
					case NJ_BAKUENRYU:
						...
						break;
					case NPC_FIREATTACK:
						skillratio += skill_lv * 350;
						break;
					case WZ_FIREPILLAR:
						skillratio += skill_lv * 450;
						break;
					...
					case AB_ADORAMUS:
						...
						break;
					// -------------------------------------------
					case WZ_VERMILION: // Raiton Kirin 7 Repetições 
						skillratio += 200 * skill_lv;
						break;
					case RAI_NAGASHI:
						skillratio += 600 * skill_lv;
					...
					case RK_SONICWAVE:
						...
						break;
					case NJ_HUUJIN:
						(skill_lv < 11 ? mestrar = 280 : mestrar = 325);
						skillratio += skill_lv * mestrar;
						break;
					// -------------------------------------------
					case SL_SMA:
		...
		if (!(nk&NK_NO_ELEFIX)) // Soul Breaker's magic portion is non-elemental. [Secret]
...
int battle_damage_area(struct block_list *bl, va_list ap) {
	...
	if (status_bl_has_mode(bl, MD_SKILL_IMMUNE) || status_get_class(bl) == MOBID_EMPERIUM || status_get_class(bl) == MOBID_HIRAISHIN)
		return 0;
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "max_lv",                             &battle_config.max_lv,                          999,     0,      MAX_LEVEL,      },
	{ "aura_lv",                            &battle_config.aura_lv,                         999,     0,      INT_MAX,        },
