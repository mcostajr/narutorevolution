...
int unit_can_move(struct block_list *bl) {
	...
	if (ud->skilltimer != INVALID_TIMER && ud->skill_id != LG_EXEEDBREAK && (!sd || !pc_checkskill(sd, NV_BASIC) || skill_get_inf2(ud->skill_id)&INF2_GUILD_SKILL))
	...
	if (sc) {
		...
		if ((sc->option & OPTION_HIDE) && (!sd || pc_checkskill(sd, NV_BASIC) <= 0))
			return 0;
	}
...
int unit_skilluse_id2(struct block_list *src, int target_id, uint16 skill_id, uint16 skill_lv, int casttime, int castcancel)
{
	...
	if( casttime > 0 ) {
		...
		if( sd && (pc_checkskill(sd, NV_BASIC) > 0 || skill_id == LG_EXEEDBREAK) )
...
static int unit_attack_timer_sub(struct block_list* src, int tid, t_tick tick)
{
	...
	if( ud->skilltimer != INVALID_TIMER && !(sd && pc_checkskill(sd, NV_BASIC) > 0) )
		return 0; // Can't attack while casting

	if( !battle_config.sdelay_attack_enable && DIFF_TICK(ud->canact_tick,tick) > 0 && !(sd && pc_checkskill(sd, NV_BASIC) > 0) ) {
