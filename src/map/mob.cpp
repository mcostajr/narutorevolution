...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	struct {
		struct party_data *p;
		int id,zeny;
		unsigned int base_exp,job_exp,hiden_exp;
	} pt[DAMAGELOG_SIZE];
	...
	if(!(type&2) && //No exp
		...
		for(i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++) {
			...
			if (map_getmapflag(m, MF_NOBASEEXP) || !md->db->hiden_exp)
				hiden_exp = 0;
			else {
				double exp = apply_rate2(md->db->hiden_exp, per, 1);
				exp = apply_rate(exp, bonus);
				exp = apply_rate(exp, map_getmapflag(m, MF_BEXP));
				hiden_exp = (unsigned int)cap_value(exp, 1, UINT_MAX);
			}
			...
			if ( ( temp = tmpsd[i]->status.party_id)>0 ) {
				...
				if( j == pnum ) { //Possibly add party.
					...
					if (pt[j].hiden_exp > UINT_MAX - hiden_exp)
						pt[j].hiden_exp = UINT_MAX;
					else
						pt[j].hiden_exp += hiden_exp;
				...
				if(base_exp || job_exp || hiden_exp) {
					if( md->dmglog[i].flag != MDLF_PET || battle_config.pet_attack_exp_to_master ) {
						...
						if (rate != 100) {
							...
							if (hiden_exp)
								hiden_exp = (unsigned int)cap_value(apply_rate(hiden_exp, rate), 1, UINT_MAX);
						}
#endif
						pc_gainexp(tmpsd[i], &md->bl, base_exp, job_exp, hiden_exp, 0);
					}
				}
		...
		for( i = 0; i < pnum; i++ ) //Party share.
			party_exp_share(pt[i].p, &md->bl, pt[i].base_exp,pt[i].job_exp,pt[i].hiden_exp,pt[i].zeny);
	...
	if(mvp_sd && md->db->mexp > 0 && !md->special_state.ai) {
		...
		pc_gainexp(mvp_sd, &md->bl, mexp,0, 0, 0);
		log_mvp[1] = mexp;
...
static void mob_load(void)
{
	...
	if( db_use_sqldbs ){
		...
		for(int i = 0; i < ARRAYLENGTH(dbsubpath); i++){
			...
			sv_readdb(dbsubpath2, "mob_db.txt", ',', 32+2*MAX_MVP_DROP+2*MAX_MOB_DROP, 32+2*MAX_MVP_DROP+2*MAX_MOB_DROP, -1, &mob_readdb_sub, silent);
