...
static void clif_getareachar_pc(struct map_session_data* sd,struct map_session_data* dstsd)
{
	...
	if( (sd->status.party_id && dstsd->status.party_id == sd->status.party_id) || //Party-mate, or hpdisp setting.
		(sd->status.guild_id && dstsd->status.guild_id == sd->status.guild_id) || //Guild-mate, or hpdisp setting.
...
void clif_parse_QuitGame(int fd, struct map_session_data *sd)
{
	...
	if( !sd->sc.data[SC_CLOAKING] && !sd->sc.data[SC_HIDING] && !sd->sc.data[SC_CHASEWALK]
		&& !sd->sc.data[SC_CLOAKINGEXCEED] && !sd->sc.data[SC_SUHIDE] && !sd->sc.data[SC_STEALTHFIELD]
...
void clif_parse_ActionRequest_sub(struct map_session_data *sd, int action_type, int target_id, t_tick tick)
{
	...
	case 0x07: // continuous attack
		...
		if (!battle_config.sdelay_attack_enable && pc_checkskill(sd, NV_BASIC) <= 0) {
