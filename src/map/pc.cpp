 ...
 * @return
 **/
void pc_gainexp(struct map_session_data *sd, struct block_list *src, unsigned int base_exp, unsigned int job_exp, unsigned int hiden_exp, uint8 exp_flag)
{
...
int pc_resetstate(struct map_session_data* sd)
{
	...
	{	// New statpoint table used here - Dexity
		...
		sd->status.status_point = statp[sd->status.base_level]; // extra 52+48=100 stat points
	}
	else
	{
		int add=0;
	...
	case SP_BASEEXP:
		{
			...
			if ((unsigned int)val < sd->status.base_exp) // Lost
				...
				pc_gainexp(sd, NULL, val - sd->status.base_exp, 0, 0, 2);
		}
		return true;
	case SP_JOBEXP:
		{
			...
			if ((unsigned int)val < sd->status.job_exp) // Lost
				...
				pc_gainexp(sd, NULL, 0, val - sd->status.job_exp, 0, 2);
		}
		return true;
	case SP_SEX:
...
int pc_itemheal(struct map_session_data *sd, int itemid, int hp, int sp)
{
	...
	if (sp) {
		bonus = 100 + (sd->battle_status.int_ << 1) * 10 + pc_checkskill(sd, AM_LEARNINGPOTION) * 5;
		// A potion produced by an Alchemist in the Fame Top 10 gets +50% effect [DracoRPG]
		if (potion_flag == 2)
			bonus += 50;
...
void pc_readdb(void) {
	...
	for(i=0; i<ARRAYLENGTH(dbsubpath); i++){
		...
		if (i == 0)
			...
			sv_readdb(dbsubpath1, "pre-re/job_db1.txt",',',6+MAX_WEAPON_TYPE,6+MAX_WEAPON_TYPE,CLASS_COUNT,&pc_readdb_job1, false);
#else
