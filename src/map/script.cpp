...
BUILDIN_FUNC(getexp)
{
	...
	pc_gainexp(sd, NULL, base, job, 0, 1);
...
BUILDIN_FUNC(setmounting) {
	...
	} else if (sd->sc.data[SC_CLOAKING] || sd->sc.data[SC_CHASEWALK] || sd->sc.data[SC_CLOAKINGEXCEED]
		|| sd->sc.data[SC_CAMOUFLAGE] || sd->sc.data[SC__FEINTBOMB] || sd->sc.data[SC_STEALTHFIELD]) {
...
BUILDIN_FUNC(getexp2) {
	...
	if (base_exp > 0)
		pc_gainexp(sd, NULL, base_exp, 0, 0, 2);
	else if (base_exp < 0)
		pc_lostexp(sd, base_exp * -1, 0);

	if (job_exp > 0)
		pc_gainexp(sd, NULL, 0, job_exp, 0, 2);
	else if (job_exp < 0)
