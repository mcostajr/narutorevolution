...
void initChangeTables(void)
{
	...
	set_sc(UH_MANGEKYOU, SC_MANGEKYOU, EFST_MANGEKYOU, SCB_NONE);
	set_sc(UH_SUSANOO, SC_SUSANOO, EFST_SUSANOO, SCB_NONE);

	set_sc(BY_BYAKUGAN, SC_BYAKUGAN, EFST_BYAKUGAN, SCB_HIT);

	set_sc(PT_1PORTAO, SC_PORTAO1, EFST_PORTAO1, SCB_BATK | SCB_WATK);

	set_sc(AK_VERDE, SC_PILULA1, EFST_PILULA1, SCB_DEF | SCB_MDEF);
	set_sc(AK_AMARELA, SC_PILULA2, EFST_PILULA2, SCB_MAXHP | SCB_REGEN);
	set_sc(AK_VERMELHA, SC_PILULA3, EFST_PILULA3, SCB_BATK | SCB_WATK | SCB_SPEED);
	...
	/* Fuuinjutsu */
	set_sc(AC_OWL, SC_OWL, EFST_OWL, SCB_DEX);
	set_sc(NJ_NEN, SC_NEN, EFST_NJ_NEN, SCB_STR | SCB_INT);
	set_sc(AC_CONCENTRATION, SC_CONCENTRATE, EFST_CONCENTRATION, SCB_AGI | SCB_DEX | SCB_SPEED);
	set_sc(MO_EXPLOSIONSPIRITS, SC_EXPLOSIONSPIRITS, EFST_EXPLOSIONSPIRITS, SCB_STR);
	add_sc(AL_RUWACH, SC_RUWACH);
	set_sc(AL_BLESSING, SC_BLESSING, EFST_BLESSING, SCB_STR | SCB_INT | SCB_DEX);
	set_sc(SO_STRIKING, SC_STRIKING, EFST_STRIKING, SCB_BATK|SCB_WATK);
	...
	/* Kugutsu */
	add_sc(HLIF_CHANGE, SC_SILENCE);
	add_sc(MH_PAIN_KILLER, SC_STUN);
...
//#endif
	...
	set_sc( KO_KYOUGAKU			, SC_KYOUGAKU		, EFST_KYOUGAKU		, SCB_NONE);
	...
	// Hyouton
	StatusChangeStateTable[SC_SUIJINHEKI]			|= SCS_NOMOVE;
	StatusChangeStateTable[SC_ICEWALL]				|= SCS_NOMOVE;
	...
	StatusChangeStateTable[SC_KYOUGAKU]				|= SCS_NOPICKITEM;
	...
	StatusChangeStateTable[SC_RECOVERCHAKRA]		|= SCS_NODROPITEM;
	StatusChangeStateTable[SC_KYOUGAKU]				|= SCS_NODROPITEM;
...
int status_damage(struct block_list *src,struct block_list *target,int64 dhp, int64 dsp, t_tick walkdelay, int flag)
{
	...
	if( hp && !(flag&1) ) {
		if( sc ) {
			...
			status_change_end(target, SC_STEALTHFIELD, INVALID_TIMER);
	...
	if (tsc) {
		...
		switch (target->type) {	// Check for chase-walk/hiding/cloaking opponents.
			case BL_PC: {
					...
					if (((tsc->option&(OPTION_HIDE|OPTION_CLOAK|OPTION_CHASEWALK)) || tsc->data[SC_CAMOUFLAGE] || tsc->data[SC_SUHIDE]) && !is_boss && (tsd->special_state.perfect_hiding || !is_detector))
						return 0;
				...
				break;
			default:
				if (((tsc->option&(OPTION_HIDE|OPTION_CLOAK|OPTION_CHASEWALK)) || tsc->data[SC_CAMOUFLAGE] || tsc->data[SC_SUHIDE]) && !is_boss && !is_detector)
					return 0;
		}
	}
...
unsigned int status_weapon_atk(struct weapon_atk wa, struct map_session_data *sd)
{
	...
	float int_ = sd->base_status.int_;
	float stat;
	...
	return wa.atk + wa.atk2 + (int)(wa.atk * (stat / 200) + weapon_atk_bonus);
}
//#endif
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	// ATK
	if (bl->type != BL_PC) {
		status->rhw.atk2 = status_base_atk_max(bl, status);
		status->rhw.atk = status_base_atk_min(bl, status);
	}
	// Matk
	status->matk_min = status_base_matk_min(bl, status);
	status->matk_max = status_base_matk_max(bl, status);
	// Hit
	stat = status->hit;
	stat += level + status->dex; /* NRO */
	status->hit = cap_value(stat, 1, SHRT_MAX);
	// Flee
	stat = status->flee;
	stat += level + status->dex;  /* NRO */
	...
	// Def2
	stat = status->def2;
	stat += status->vit + (status->agi/5); /* NRO */
	status->def2 = cap_value(stat, 0, SHRT_MAX);
	// Mdef2
	stat = status->mdef2;
	stat += status->int_ + (status->vit>>1) + (status->agi/5); /* NRO */
	status->mdef2 = cap_value(stat, 0, SHRT_MAX);
#endif
	...
	if (flag&16 && mbl) { // Max HP setting from Summon Flora/marine Sphere
		...
		if (ud) { 
				...
				case NPC_GROUNDATTACK:
			{
				...
				if (!mstatus)
					break;
				status->max_hp = mstatus->hp/2;
				status->def = mstatus->def;
				status->mdef = mstatus->mdef;
				status->rhw.atk = (mstatus->matk_max*10) * (ud->skill_lv);
				status->rhw.atk2 = (mstatus->matk_max*5) * (ud->skill_lv);
				status->str = mstatus->str + (ud->skill_lv*50);
				status->agi = mstatus->agi + (ud->skill_lv*10);
				status->vit = mstatus->vit + (ud->skill_lv*50);
				status->int_ = mstatus->int_ + (ud->skill_lv*50);
				status->dex = mstatus->dex + (ud->skill_lv*60);
				status->luk = mstatus->luk + (ud->skill_lv*50);
				status->aspd_rate = 5000/ud->skill_lv;
				break;
			}
			case KN_C1:
			{
			...
			case TY_DOKI1:
			{
				...
				status->def = mstatus->def;
				status->mdef = mstatus->mdef;
				status->rhw.atk = (mstatus->batk) + (ud->skill_lv*2000);
				status->rhw.atk2 = (mstatus->batk) + (ud->skill_lv*2500);
				status->str = mstatus->str + (ud->skill_lv*50);
				status->agi = mstatus->agi + (ud->skill_lv*50);
				status->vit = mstatus->vit + (ud->skill_lv*50);
				status->int_ = mstatus->int_ + (ud->skill_lv*50);
				status->dex = mstatus->dex + (ud->skill_lv*200);
				status->luk = mstatus->luk + (ud->skill_lv*50);
				status->aspd_rate = 1000/ud->skill_lv;
				break;
			}
			case TY_DOKI2:
			{
			...
			case IB_SALAMANDRA:
			{
				...
				status->def = mstatus->def;
				status->mdef = mstatus->mdef;
				status->rhw.atk = (mstatus->batk) + (ud->skill_lv*10000);
				status->rhw.atk2 = (mstatus->batk) + (ud->skill_lv*15000);
				status->str = mstatus->str + 200;
				status->agi = mstatus->agi + 200;
				status->vit = mstatus->vit + 200;
				status->int_ = mstatus->int_ + 200;
				status->dex = mstatus->dex + 600;
				status->luk = mstatus->luk + 200;
				status->aspd_rate = 500;
				break;
			}
				case AM_SPHEREMINE:
...
void status_calc_regen(struct block_list *bl, struct status_data *status, struct regen_data *regen)
{
	...
	val = (status->agi*10) + (1 + (status->vit / 5) + (status->max_hp/200));
...
void status_calc_bl_main(struct block_list *bl, /*enum scb_flag*/int flag)
{
	...
	**/
	...
	if(flag&SCB_MATK) {
		status->matk_min = status_base_matk_min(bl, status) + (sd ? sd->bonus.ematk : 0);
		status->matk_max = status_base_matk_max(bl, status) + (sd ? sd->bonus.ematk : 0);
...
static unsigned short status_calc_speed(struct block_list *bl, struct status_change *sc, int speed)
{
	...
	if( sd && sd->ud.skilltimer != INVALID_TIMER && (pc_checkskill(sd, NV_BASIC) > 0 || sd->ud.skill_id == LG_EXEEDBREAK) ) {
		...
		// GetMoveSlowValue()
		if( sd && sc->data[SC_HIDING] && pc_checkskill(sd, NV_BASIC) > 0 )
			val = 0;
...
/*
	Status Regen Chakra
*/
int status_chakra(struct block_list* bl)
{
	int chakra = 100;
	...
	if (sd) {
		if (pc_checkskill(sd, NJ_NINPOU) == 10)
			chakra += 5 * status->max_sp / 100;
	}
...
//		if (status_get_race2(bl) == RC2_GVG && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	case SC_INVINCIBLEOFF:
		...
		break;
	case SC_WHITEIMPRISON:
		clif_specialeffect(bl, 111, AREA);
		break;
	case SC_FEAR:
		status_change_end(bl, SC_BLIND, INVALID_TIMER);
		break;
	...
	*/
	...
	case SC_OWL:
		...
		break;
	case SC_NEN:
		val2 = pc_checkskill(sd, NJ_NEN) > 10 ? 3*val1 : 2 * val1;
		break;
	case SC_CONCENTRATE:
		val2 = 1 + val1;
		val3 = 20 + val1 * 5;
	...
	// Portoes
	case SC_PORTAO1:
		if (sc->data[SC_PORTAO1] && sc->data[SC_PORTAO1]->val1 >= 2)
			status_heal(bl, status->max_hp * 20 / 100, 0, 1);
		t_tickime = 1000;
		val4 = tick / t_tickime;
		break;
	...
	// --------------------------------------------------------------------
		...
		case SC_WEAPONPERFECTION:
			{
				struct map_session_data * s_sd = BL_CAST(BL_PC, src);
				 if (type == SC_ADRENALINE2 || type == SC_ADRENALINE) {
		...
		case SC_UTSUSEMI:
			...
			val3=skill_get_blewcount(KO_ZANZOU, val1); // knockback value.
			break;
		...
		case SC_STRIKING:
			val2 = 10 * val1;
			val3 = 11 - val1;// spcost = 6 - level (lvl1:10 ... lvl 10: 1)
			val4 = tick / 1000;
			t_tickime = 1000; // [GodLesZ] tick time
			break;
		case SC_WARMER:
		...
		case SC_ENERGY_DRINK_RESERCH:
			...
			break;
		case SC_KYOUGAKU:
			clif_status_change(bl,EFST_ACTIVE_MONSTER_TRANSFORM,1,0,30135,0,0);
			break;
		case SC_KAGEMUSYA:
			val2 = tick/1000;
			val3 = val1 * 2;
...
TIMER_FUNC(status_change_timer){
	...
	case SC__INVISIBILITY:
		if( !status_charge(bl, 0, (25 - 2 * sce->val1) * status->max_sp / 100) ) // 6% - skill_lv.
			break;
		sc_timer_next(1000 + tick);
		return 0;
